...
- **Final QA — partial pass** (Phase 8 Slice 3): QA checklist drafted + committed at `docs/superpowers/qa-checklists/2026-05-13-phase-8-final-qa.md`. First-session walkthrough surfaced three deferrable items captured with their own spec stubs: RSS link UX (raw-XML on click; pretty-render via XSL deferred), garden path-log retrieval (consent banner has no UI consumer — `docs/superpowers/specs/2026-05-13-garden-path-log-retrieval-design.md`), and a mobile page-sidebar strip pinning bug at viewport widths < 1220 px. **The strip bug is resolved** (2026-05-13, commit `bab359d`): root cause was `.site-nav` lacking `flex-wrap`, which forced horizontal document overflow at narrow viewports and broke Firefox's `position: sticky` paint math. Fixed via `flex-wrap: wrap` on `.site-nav` + `html { overflow-x: clip }` (clip not hidden — clip does not create a scroll container so sticky descendants keep working). Resolution write-up in the spec stub.
