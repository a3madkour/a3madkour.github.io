...
| `js/entry-recipes.js` | `recipes.<hash>.js` | `.Section == "recipes"` | `recipe-scale.js` (unit-aware quantity scaler) + `filter-chips.js`; handles both single-page rail binding and index filter chips |
...
- **Top nav** (locked): Essays / Garden / Research / Works / Library / Streams / Recipes / About. Active item gets `aria-current="page"` via `hasPrefix` match. (Streams added 2026-05-19; Recipes added 2026-07-06; now 8 items.)
...
**Recipes** (`content/recipes/<slug>/index.md`) — enforced by `tools/check_recipes_fixtures.py` (34th pair) + `tools/check_recipes_links.py` (35th pair). Required: `title, draft, servings, ingredients, steps`. Optional: `summary, tags, cuisine, category, sources, prep_time, cook_time`. `ingredients` and `sources` are flow-style YAML objects; `qty` on an ingredient may be `null` (uncountable items). Spec: `docs/superpowers/specs/2026-07-06-recipe-slice-1-render-download-design.md`.
...
The audit remediation roadmap is fully closed (R1–R6.3 all shipped). **Current active work: Recipes section** — Slice 1 (site render + download) is on branch `recipes-slice-1`; Slice 2 (org authoring + lint + export) and Slice 3 (meal-prep planner) are queued stubs. See memory `project_recipe_section.md`.
