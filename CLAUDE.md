...
- **Citation export** (post-Phase-8 polish, 2026-05-13): page-level cite metadata + per-reference cite affordances on every citable single page (essays / garden / research themes & questions / works games & music & poetry). Highwire `<meta>` tags emitted in `<head>` for Zotero auto-detect. `<dialog>`-driven modal opens from a "Cite this page" link beneath the title; five tabs (BibTeX / APA / Chicago / MLA / RIS) with copy + per-format download (`citekey.bib`/`.txt`/`.ris`). Half B: each `<li>` in `essay-references.html` carries an inline `.ref-cite-actions` strip (BibTeX / APA / .ris quick-copy buttons + "Full citation" link) which also rides inside the existing citation hover-card via its `innerHTML` clone (free). With JS off, a `<section id="cite-this">` at page bottom exposes all 5 formats under native `<details>` and Zotero's meta tags still work. Build-time pipeline: `cite/normalize-{page,ref}.html` produce a citation dict; `cite/fmt-{bibtex,apa,chicago,mla,ris}.html` consume it; `cite/data-blob.html` emits a single `<script id="cite-data">` JSON blob with `self` + `refs` (one entry per `Page.Scratch.cite-keys` slug the cite shortcode populated). **Gotcha**: the JSON-blob embed MUST pipe `jsonify | safeJS` — without `safeJS` Hugo's HTML-context auto-escape double-encodes the result and the runtime parses a string instead of a dict (same trap CLAUDE.md flags for graph-data). New `cite.<hash>.js` bundle (~2.5 KB) loaded on citable pages only via `entry-cite.js`. CSS §43 — token-driven; bottom-sheet at ≤720px. 17th linter pair (`tools/check_cite_meta.py` + sibling) post-build asserts every citable page has all 5 `citation_*` meta tags, a valid `<script id="cite-data">` blob with `self.citekey` matching `madkour-<year>-<slug>`, all 5 format keys non-empty in self.formats, every refs key existing in `data/citations.yaml`, and a `<section id="cite-this">` static fallback. `tools/check_citations.py` extended to whitelist 7 new optional fields (doi/publisher/volume/issue/pages/isbn/type) — required-set unchanged. CI workflow grows by 2 named steps (44 → 46).
...
Recommended sequencing across remaining queued work: **Time-synced poetry (next up — spec drafted, plan still needed)** → Phase 3 Slice 1 (garden publish) → Phase 3 Slice 2 (essay publish) → Multi-target export → Streams section.
