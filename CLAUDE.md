...
- **Final QA — partial pass** (Phase 8 Slice 3): QA checklist drafted + committed at `docs/superpowers/qa-checklists/2026-05-13-phase-8-final-qa.md`. First-session walkthrough surfaced three deferrable items captured with their own spec stubs: RSS link UX (raw-XML on click; pretty-render via XSL deferred), garden path-log retrieval (consent banner has no UI consumer — `docs/superpowers/specs/2026-05-13-garden-path-log-retrieval-design.md`), and a mobile page-sidebar strip pinning bug at viewport widths < 1220 px (`docs/superpowers/specs/2026-05-13-mobile-page-sidebar-strip-bug-design.md`). The strip bug is the blocker — multiple CSS fixes failed in-session; DOM/paint APIs report the strip is pinned at viewport top, but it visually disappears mid-scroll on real phone too. CSS reverted to pre-fix state. Remaining checklist items (keyboard nav §1.1–1.5+1.7–1.9, SR walkthrough §2, CB sim §3, mobile audit §4 excluding the strip cluster, perf §5) are deferred to a follow-up session paired with the strip bug investigation.
...
- **Phase 8 follow-up: mobile page-sidebar strip bug + finish QA walkthrough.** See `docs/superpowers/specs/2026-05-13-mobile-page-sidebar-strip-bug-design.md`. Pair the bug investigation with resuming the QA checklist for items still unchecked.
