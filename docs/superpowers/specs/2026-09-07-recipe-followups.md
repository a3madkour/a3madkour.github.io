...
**Status as of 2026-09-08: every row is closed.** Rows RF1.1-RF3.2 were closed
in the 2026-09-07 run; RF2.5 was withdrawn as a false positive; RF3.3 and RF3.4
closed 2026-09-08. Each row keeps the evidence it was filed with, so the
reproductions below describe the tree *at filing time*, not today's.
...
| RF3.4 | ~~**Two checked pairings sit ~0.1 above the AA bar.**~~ **CLOSED — and the row understated it.** A full sweep of all 21 pairings x 2 modes put the *thinnest* pair at `color-warn on color-stone` / `color-stone on color-warn`, **4.55:1** — tighter than either pairing the row named, and unmentioned by it. Full thin set as found: warn/stone both directions **+0.05**; burgundy against tile and paper, four directions, **+0.06** (dark); `ink-fade on stone` **+0.12** (light); `green-soft on stone` **+0.26** (light). | `python3 tools/check-contrast.py` prints each pairing's margin and closes with the sub-`TIGHT_MARGIN` set, so the sweep is reproducible from the tool rather than from this table. Mutation-proven twice: `--color-green-soft` at `#507159` enters the block at `+0.19` in sort order (pre-nudge threshold 0.25), and post-nudge `--color-warn` at `#9e5819` enters at `+0.17` — both with the run still exiting 0. | Done, both halves. **Reporting:** the margin now lives in the gate's output. **Palette:** three hue-preserving nudges — light `--color-warn` `#a05a1a`->`#9c5719` (4.55->4.76), light `--color-ink-fade` `#6e6a64`->`#6c6862` (4.62->4.76), dark `--color-burgundy` `#de6f7d`->`#df7481` (4.56->4.75, edited in both dark blocks so `check_dark_tokens.py` stays green). Every knock-on moved upward; no pairing changed verdict. Floor is now `+0.25` dark / `+0.26` light. `TIGHT_MARGIN` was then set to **0.20**, below the palette's target, so an empty report means clear-of-the-line rather than on-it. |
