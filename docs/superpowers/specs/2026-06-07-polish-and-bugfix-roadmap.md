...
**Date:** 2026-06-07
**Status:** Active. Each tier maps to one or more future sessions. **Tiers 1, 2, 4, 5, 6 closed; Tier 3 is human-driven; Tier 7 partially closed (7.1 shipped 2026-06-12; 7.2 still trigger-gated); Tier 8 fully closed 2026-06-12 (8.2 org→synced-poetry; 8.1 sub-project E explorables).** Tier 1 closed 2026-06-07; Tier 2 fully closed 2026-06-11 (2.1 + 2.5 shipped 2026-06-07/08; 2.4 shipped 2026-06-08 fixture-driven; 2.2 shipped 2026-06-08 fixture-driven; 2.3 shipped 2026-06-11 fixture-driven — 2.2/2.3/2.4 closed via the trigger-gated fixture rule per [[feedback-trigger-gated-make-fixture]]); Tier 3 human-driven (manual QA); Tier 4 closed 2026-06-08; Tier 5 closed 2026-06-08 (5.1 + 5.2 shipped; preview-section deferred as 5.3 stub); Tier 6 closed-by-deferral 2026-06-08 (6.1 demoted back to the deferred-features registry — see CLOSED block below); Tier 8.2 closed 2026-06-12 (org → synced-poetry export shipped end-to-end with smoke-test-poem live).
...
| 2.2 | ✓ **D.1 cross-reference auto-formatting** 2026-06-08. New `ref-block` shortcode resolves via `$page.Scratch` (each of the 11 numbered AMS blocks writes `block-label-{id} → "Kind N"` right after counter increment); backward-refs render server-side; forward / broken refs fall back to bare id + `.ref-block-unresolved` warning style. Hugo doesn't do true two-pass — author either places refs after the target block, or uses manual `[Theorem 1](#thm-foo)` for forward cases. Fixture-first: `content/essays/example-blocks-crossref/` exercises backward / forward / broken / all 11 numbered kinds. → [project-tier-2-2-complete](../../../.claude/memory/project_tier_2_2_complete.md) | n/a (shipped) | New shortcode in `layouts/shortcodes/`. Source: D.1 follow-up #1 in [project-d1-complete](../../../.claude/memory/project_d1_complete.md). |
| 2.3 | ✓ **D.1 section-prefixed numbering** 2026-06-11. Per-essay opt-in via frontmatter `block_numbering: "section-prefixed"`; `<main>` emits `data-block-numbering` attribute when set; client-side `block-renumber.js` walks H2s + `.block-*` containers, rewrites `Kind N` → `Kind M.N` and updates matching `.ref-block` text. Hugo cannot do this server-side (shortcodes run before Goldmark sees H2s). Theorem-family shares counter per-section; each non-family kind has its own. Fixture-first: `content/essays/example-long-numbering/` (5 H2 sections, 14 blocks, opted-in). → [project-tier-2-3-complete](../../../.claude/memory/project_tier_2_3_complete.md) | n/a (shipped) | New JS module loaded via `entry-essay.js`; baseof.html attribute emission. |
...
| # | Item | Source |
|---|---|---|
| 8.1 | ✓ **Sub-project E — explorable explainables** (Phase 3 final piece). Per-page interactive widgets + per-page JS bundle convention. Shipped 2026-06-12 (site `fbde0f7..5d742dd`, 22 commits: spec + plan + 20 implementation). 28th linter pair (6 rules) + CSS §49 + 13th JS bundle (per-essay dynamic loop) + 2 library kinds (ReactiveValue, ReactiveChart) + runtime + shortcode upgrade + 1 fixture + example-one migration. → [project-tier-8-1-complete](../../../.claude/memory/project_tier_8_1_complete.md) | `docs/superpowers/specs/2026-06-12-sub-project-e-explorables-design.md` + `docs/superpowers/plans/2026-06-12-sub-project-e-explorables.md` |
| 8.2 | ✓ **Org → synced-poetry export.** Phase 3 Essay/poetry publish emits the shipped `[mm:ss]` + `audio_url` contract. Shipped 2026-06-12 (dotfiles `db9da62..a9a1acb`, 17 commits; site `e81e227..0a9be38`, 9 commits). New `works-poetry` peer handler + `#+AUDIO:` keyword + bundled-audio asset pipeline + lastmod cascade + summary-keyword reader + Case C `\\[mm:ss]` post-export collapse. Suite 662 → 692 (+30 ert); 1 subprocess integration test; smoke-test-poem live as bundled-audio fixture. Two real-corpus bugs caught during closure: `poetry-dir` defcustom default + missing `#+HUGO_SUMMARY:` reader. | `docs/superpowers/specs/2026-05-19-org-synced-poetry-export.md` + `docs/superpowers/plans/2026-06-12-org-synced-poetry-export.md` |
