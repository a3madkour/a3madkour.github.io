...
| 2.5 | ☐ **Cite-modal Source / Related-note pill colors look off.** Author flagged during the 2026-06-07 Tier 2.1 dev-server spot-check: in light mode the pill text reads burgundy-ish (expected: navy steel `#1e4060`), in dark mode the pill text reads bright sky-blue (`#7eafd0` from `--color-steel`) which feels jarring against the dark `--color-paper` `#2a2a2a` modal background. Linked to `.cite-modal-source` and `.cite-modal-note` rules in CSS §43. Two possible roots: (a) CSS cascade override that's coloring text burgundy in light mode (suspected `a {}` global at line 253 winning somehow — needs DevTools confirmation), or (b) design-choice mismatch (sky-blue is genuinely loud in dark mode against the paper bg). | Author appetite (filed 2026-06-07 — pre-existing, surfaced during Tier 2.1 spot-check; CSS not touched by that slice) | Step 1: ask author for DevTools "Computed" color of `.cite-modal-source` in both modes to distinguish (a) vs (b). Step 2: if (a), find the overriding selector and scope it. If (b), dial dark-mode `--color-steel` softer or introduce a modal-scoped token. Source: 2.1 spot-check follow-up in [project-anchor-affordance-complete](../../../.claude/memory/project_anchor_affordance_complete.md). |
