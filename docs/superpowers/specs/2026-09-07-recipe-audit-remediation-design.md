...
| RC4.6 | `image` is never validated — not shape, not existence — and is never rendered to a human, so breakage is invisible | Validate in `check_recipes_links.py`, mirroring the **three** shapes `schema-recipe.html` resolves: absolute `http(s)` URL (passed through), root-relative `/x` (resolves under `static/`), or bundle-relative (must exist beside `index.md`). The two implementations must use identical branch keys, so every divergence falls in the safe direction — linter stricter than template, never the reverse |
