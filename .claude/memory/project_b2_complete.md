...
metadata: 
  node_type: memory
  type: project
  originSessionId: bba3afe8-091e-4023-bcb3-bc7b6a67953f
---
...
7. **Cover assets aren't auto-extracted from the library org files.** Today covers must be manually committed to `static/library/covers/` and then referenced by bare slug in `:COVER_FILE:`. User wants the publisher to extract image links / file paths from the library org file and auto-copy them into `static/library/covers/`, rewriting the YAML to the placed slug. Spec'd implementation is undecided (scan body for `[[file:...]]` org links? accept absolute/relative paths in `:COVER_FILE:` and copy on publish? hybrid?). Flagged during Task 17 spot-check setup 2026-05-31; folds into a future B.2.x patch or B.2.1 slice depending on scope.
8. **`#+begin_comment` does NOT shield depth-1 headings.** In org-mode, a `*` at column 0 breaks any open block. The scaffolds' "EXAMPLE — DELETE OR REPLACE BELOW" headings inside the comment block were emitted as live entries on the first publish-living run during Task 17. Anyone who reads a scaffold and uncomments `#+HUGO_PUBLISH: t` without first deleting the example block will inadvertently publish the example. Scaffold redesign options: (a) move examples into `#+begin_src org` blocks (no `*` at col 0); (b) indent example headings under a sentinel depth-2 wrapper; (c) move examples into a separate `library-EXAMPLES.org` that never gets `#+HUGO_PUBLISH: t`. Logged 2026-05-31.
9. **Org-tag spec disallows hyphens; site tag convention is kebab-case.** Org-mode tags must match `[A-Za-z0-9_@]+`. The publisher silently rejects tag strings containing hyphens — they stay in the heading title, slug derivation mangles, emitted `tags: []`. Spot-check hit this with `:sci-fi:example:` on `Example Series Two`. Site convention everywhere else uses kebab-case (`procedural-narrative`, `sci-fi`, `non-fiction`). Resolution: either (a) accept no-hyphen tags in org and translate on emit (`scifi` → `sci-fi`), or (b) switch all site tag taxonomy to no-hyphen, or (c) configure custom `org-tag-re` to allow hyphens. Logged 2026-05-31.
10. **No filesystem-mtime fallback when git-mtime returns nil.** `--git-mtime-of-file` only works for tracked files; `~/org/notes/` is not git-tracked, so every stub spot-check entry got `last_modified` unpopulated and failed `check_library_fixtures`. Workaround: hand-set `:LAST_MODIFIED:` per entry — defeats the idempotence the helper was meant to deliver. Cleaner fix: cascade `:LAST_MODIFIED:` drawer → git mtime → filesystem mtime → today. Logged 2026-05-31.
11. **filter-chips.yaml, library-shelves.yaml, and tools/check_smoke.py are coupled to library slugs/tags.** When B emits a different slug/tag set (real corpus shift, or stub-vs-real swap), all three break: filter-chips primary_tags drift, shelves item lists drift, smoke URL list drift. Spot-check fixed all three by hand. Future option: have publish-living emit a "what changed" report, or have these three configs auto-derive from emitted YAML, or have linters auto-suggest fixes. Logged 2026-05-31.
12. **Music tiles (1:1 square) look bad next to other media tiles (140:190 portrait) in a shared shelf row.** `data/library-media.yaml` declares `listening.cover_aspect: square` while reading/playing/watching are `portrait`. The old hand-curated fixture shelves silently respected this — music never coexisted with other media in the same shelf. Spot-check restructure also keeps shelves medium- or aspect-homogeneous. Real fix is either: (a) normalize all aspects, (b) make tiles align on a baseline regardless of aspect, or (c) bake the "don't mix aspects" rule into `check_library_shelves.py`. Surfaced 2026-05-31 by user feedback during dev-server preview.
