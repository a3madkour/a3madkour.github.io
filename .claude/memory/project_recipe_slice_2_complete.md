...
**REMAINING (user-gated, NOT done):** publish through the author's REAL org-roam (so id/slug/url resolve for real, not stubbed) + push. Both repos unpushed (site push = live deploy). Slice 3 (meal-prep planner) still a separate deferred project.
